...
export const metadata: Metadata = {
  title: "Bilgi Merkezi",
  description: "Retim Restorasyon bilgi merkezi; yapı onarımı, mantolama, yalıtım ve restorasyon konularında detaylı içerikler.",
};
        ...
        <div className="container-main">
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)] lg:items-start">
            ...
            <div className="space-y-8">
              {categories.map((category) => (
                <section key={category.id} id={category.id} className="rounded-sm border border-retim-gray-dark bg-white p-6 shadow-soft md:p-8">
                  <h2 className="text-xl font-bold text-retim-navy">{category.title}</h2>
                  <div className="mt-6 space-y-4">
                    {category.items.map((item) => (
                      <article key={item.question} className="rounded-sm border border-retim-gray-dark/70 bg-retim-gray/50 p-4">
